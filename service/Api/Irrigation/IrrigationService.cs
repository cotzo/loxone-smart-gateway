...
public sealed class IrrigationService
{
    ...
    public IrrigationService(IConfiguration configuration, OpenMeteoForecastClient forecast)
    {
        ...
        foreach (var zone in _options.Zones)
            _state.ZoneDeficitMm.TryAdd(zone.Id, 0);
    }
    ...
    public async Task AddObservationAsync(WeatherObservation observation, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var retentionCutoff = receivedAt.AddDays(-8);
    ...
    public async Task<IrrigationResult> CalculateAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            AccumulateCompletedDays(DateTimeOffset.UtcNow);
            observations = [.. _state.Observations];
            balances = new Dictionary<string, double>(_state.ZoneDeficitMm, StringComparer.OrdinalIgnoreCase);
        }
        finally { _gate.Release(); }
        ...
        var irrigate = zones.Any(x => x.RuntimeSeconds > 0);
        return new IrrigationResult(irrigate, localDataComplete, Math.Round(et0, 2), Math.Round(rain24, 2),
            Math.Round(rain72, 2), Math.Round(forecast.RainMm, 2), Math.Round(forecast.Et0Mm, 2),
            Math.Round(maxDeficit, 2), zones, now);
    }

    private void AccumulateCompletedDays(DateTimeOffset nowUtc)
    {
        TimeZoneInfo tz;
        try { tz = TimeZoneInfo.FindSystemTimeZoneById(_options.TimeZoneId); }
        catch { tz = TimeZoneInfo.Utc; }

        var today = DateOnly.FromDateTime(TimeZoneInfo.ConvertTime(nowUtc, tz).DateTime);
        var firstObservation = _state.Observations.Where(x => x.Timestamp is not null).OrderBy(x => x.Timestamp).FirstOrDefault();
        if (firstObservation?.Timestamp is null) return;
        ...
        while (next < today)
        {
            var dayObs = _state.Observations
                .Where(x => x.Timestamp is not null && DateOnly.FromDateTime(TimeZoneInfo.ConvertTime(x.Timestamp.Value, tz).DateTime) == next)
                .OrderBy(x => x.Timestamp).ToList();
            if (dayObs.Count >= 2)
            {
                var et0 = CalculateEt0(dayObs, _options);
                var startUtc = TimeZoneInfo.ConvertTimeToUtc(next.ToDateTime(TimeOnly.MinValue), tz);
                var rain = RainDelta(WindowWithBaseline(_state.Observations.OrderBy(x => x.Timestamp).ToList(), new DateTimeOffset(startUtc, TimeSpan.Zero))
                    .Where(x => x.Timestamp < new DateTimeOffset(TimeZoneInfo.ConvertTimeToUtc(next.AddDays(1).ToDateTime(TimeOnly.MinValue), tz), TimeSpan.Zero)).ToList());
                var effectiveRain = rain * _options.EffectiveRainFactor;
                foreach (var zone in _options.Zones)
                {
                    _state.ZoneDeficitMm.TryGetValue(zone.Id, out var current);
                    var demand = et0 * Math.Max(0, zone.Exposure);
                    _state.ZoneDeficitMm[zone.Id] = Math.Max(0, current + demand - effectiveRain);
                }
            }
    ...
    private static bool HasComplete24HourCoverage(IReadOnlyList<WeatherObservation> o, DateTimeOffset now, IrrigationConfiguration options)
    {
    ...
    private static double RainDelta(IReadOnlyList<WeatherObservation> o)
    {
        if (o.Count < 2) return 0;
        double total = 0;
        for (var i = 1; i < o.Count; i++)
        {
            var delta = o[i].RainfallMm - o[i - 1].RainfallMm;
            total += delta >= 0 ? delta : o[i].RainfallMm;
        }
        return total;
    }
    ...
    {
        if (o.Count < 2) return 0;
        var tMin = o.Min(x => x.TemperatureC); var tMax = o.Max(x => x.TemperatureC); var tMean = o.Average(x => x.TemperatureC);
        ...
        var vpd = Math.Max(0, es - ea); var pressureKpa = o.Average(x => x.PressureHpa) / 10.0;
        var gamma = 0.000665 * pressureKpa; var delta = 4098 * SaturationVapourPressure(tMean) / Math.Pow(tMean + 237.3, 2);
        var wind2m = o.Average(x => x.WindSpeedKmh) / 3.6; var rs = IntegrateSolarMjM2(o, options.LuxPerWattM2);
        var day = o[^1].Timestamp?.DayOfYear ?? DateTimeOffset.UtcNow.DayOfYear; var ra = ExtraterrestrialRadiation(options.Latitude, day);
        var rso = (0.75 + 2e-5 * options.ElevationM) * ra; var rns = 0.77 * rs; const double sigma = 4.903e-9;
        var cloud = rso > 0 ? Math.Clamp(1.35 * Math.Clamp(rs / rso, 0, 1) - 0.35, 0, 1) : 0;
        var rnl = sigma * (Math.Pow(tMax + 273.16, 4) + Math.Pow(tMin + 273.16, 4)) / 2.0 * (0.34 - 0.14 * Math.Sqrt(Math.Max(0, ea))) * cloud;
    ...
    {
        if (luxPerWattM2 <= 0) return 0; double joules = 0;
        for (var i = 1; i < o.Count; i++)
        {
            ...
            joules += (Math.Max(0, o[i - 1].LightLux / luxPerWattM2) + Math.Max(0, o[i].LightLux / luxPerWattM2)) / 2.0 * seconds;
        }
    ...
    {
        var phi = latitudeDegrees * Math.PI / 180.0; var dr = 1 + 0.033 * Math.Cos(2 * Math.PI / 365 * dayOfYear);
        var sd = 0.409 * Math.Sin(2 * Math.PI / 365 * dayOfYear - 1.39); var ws = Math.Acos(Math.Clamp(-Math.Tan(phi) * Math.Tan(sd), -1, 1));
        const double gsc = 0.0820;
        return 24 * 60 / Math.PI * gsc * dr * (ws * Math.Sin(phi) * Math.Sin(sd) + Math.Cos(phi) * Math.Cos(sd) * Math.Sin(ws));
    }

    private IrrigationState LoadState()
    {
        try { return File.Exists(_options.StateFile) ? JsonSerializer.Deserialize<IrrigationState>(File.ReadAllText(_options.StateFile)) ?? new() : new(); }
        catch (Exception ex) { Log.Warning(ex, "Unable to load irrigation state from {file}", _options.StateFile); return new(); }
    }

    private async Task PersistAsync(CancellationToken cancellationToken)
    {
        var directory = Path.GetDirectoryName(_options.StateFile); if (!string.IsNullOrWhiteSpace(directory)) Directory.CreateDirectory(directory);
        var temp = _options.StateFile + ".tmp"; await File.WriteAllTextAsync(temp, JsonSerializer.Serialize(_state), cancellationToken); File.Move(temp, _options.StateFile, true);
    }
}
