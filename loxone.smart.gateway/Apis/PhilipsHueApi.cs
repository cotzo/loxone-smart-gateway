...
{
    public static async Task SetLight(HttpContext context, string id, string ip, string accessKey, PhilipsHueLightType type, int transitionTime)
    {
        int value = await GetInputValue(context.Request.Body);
        string commandBody = string.Empty;
        
        switch (type)
        {
            case PhilipsHueLightType.RGB:
                commandBody = value < 200000000 ? GetRgbCommand(value, transitionTime) : GetTunableCommand(value, transitionTime);
                break;
        ...
        await client.PutAsync($"https://{ip}/clip/v2/resource/{type.ToString().ToLower()}/{id}", new StringContent(commandBody));
    }

    private static async Task<int> GetInputValue(Stream stream)
    {
        using var reader = new StreamReader(stream);
        var postData = await reader.ReadToEndAsync();
        return int.Parse(postData);
    }
    ...
    private static string GetDimCommand(int value, int transitionTime)
    {
        ...
        {
            return $"{{\"on\": {{\"on\": true}}, \"dimming\": {{\"brightness\": {value}}}, \"dynamics\": {{\"duration\": {transitionTime}}}}}";
        }
    }
